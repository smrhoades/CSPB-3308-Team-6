...
def test_contacts(client, auth):
    ...
    assert 'id' in contact1
    assert 'user_name' in contact1
    assert 'uuid' in contact1
    
    # Check that the correct contacts have been retrieved
    assert 'test2' in contact1['user_name']
    assert 'test3' in contact2['user_name']
    ...
    for i in range(len(messages)):
        assert messages[i]['user_from_name'] in [contact1['user_name'], contact2['user_name']] or messages[i]['user_to_name'] in [contact1['user_name'], contact2['user_name']]
